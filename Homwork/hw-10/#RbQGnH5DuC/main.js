...
//
// }
function addToLocalStorage(// створюємо функцію з дженеріком T
// T — це тип даних, який буде переданий пізніше
arrayName, // назва ключа localStorage наприклад 'users'
objToAdd // об'єкт який треба додати в масив тип цього об'єкта = T
) {
    const item = localStorage.getItem(arrayName); // дістаємо дані з localStorage, item буде string або null
    if (!item) {
        // якщо такого ключа немає викидаємо помилку
        throw new Error('there is no array');
    }
    const arrayParse = JSON.parse(item); // JSON.parse перетворює string у масив, T[] означає масив об'єктів типу T
